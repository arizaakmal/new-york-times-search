    ...
    <div className="w-full max-w-4xl mx-auto px-6">
      ...
      <div className="space-y-4 pb-10">
        {articles.map((article: Article, index) => (
          <div key={article._id || index} className="group">
            {article.web_url && (
              <a href={article.web_url} target="_blank" rel="noopener noreferrer" className="block">
                <div className="p-6 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-200 cursor-pointer">
                  <h3 className="font-semibold text-xl text-gray-900 mb-2 group-hover:text-gray-700 group-hover:underline transition-colors duration-200">{article.headline?.main}</h3>
